...
【优先级规则】：
1. 1. 问题含文件名称/路径或需解析文件：
       - 文本文件（.txt）→["text_agent"]
       - PDF 文件（.pdf）→["pdf_agent"]
       - PPTX 文件（.pptx/.ppt）→["pptx_agent"]
       - Excel 文件（.xls/.xlsx）→["excel_tools"]
       - 音频文件（.mp3/.wav/...）→["media_agent"]
       - 视频文件（.mp4/.avi/...）→["video_agent"]
       - 图片文件（.jpg/.png/.bmp/...）→["image_agent"]
       - 其他或无法识别→["multi_file_tools"]
