...
# # -------------------------- Master Agent 工作流 --------------------------
# # OxyGent-main/agent.py:181 起
# async def master_workflow(oxy_request: OxyRequest):
#     # 优先读取 master-level query，若为空则回退到普通 query
#     user_query = oxy_request.get_query(master_level=True) or oxy_request.get_query()

#     # 保留调用时透传进来的文件字段（既支持 shared_data 也支持 arguments）
#     file_name = (
#         oxy_request.get_shared_data("file_name")
#         or oxy_request.arguments.get("file_name", "")
#     )

#     # 先用 intent_agent 判断要调用哪些子 agent
#     intent_resp = await oxy_request.call(
#         callee="intent_agent",
#         arguments={"query": user_query, "file_name": file_name} if file_name else {"query": user_query},
#     )
#     agents_to_call = getattr(intent_resp, "output", ["external_search_agent"])

#     # 构造统一的 arguments，确保文件信息一并传递
#     base_arguments = {"query": user_query}
#     if file_name:
#         base_arguments["file_name"] = file_name

#     # 并行调用所有子 agent
#     tasks = [
#         oxy_request.call(callee=agent, arguments=base_arguments)
...
# async def intent_workflow(oxy_request: OxyRequest):
#     query = oxy_request.get_query() or ""
#     file_name = oxy_request.arguments.get("file_name", "")

#     # 把判定任务交给 default_llm
#     prompt = f"""你是意图识别助手，请根据用户输入决定下一步调用哪个智能体。
# 输入：
# {query}
# 若提到了文件/路径（例如 {file_name}），优先输出 ["multi_format_agent"]；
# 时间问题 → ["time_agent"]；
# 库存问题 → ["inventory_agent"]；
# 订单问题 → ["delivery_agent"]；
# 网页/京东商品/URL → ["external_search_agent"]；
# GitHub → ["github_agent"]；
# 需要实时搜索 → ["external_search_agent"]；
# 否则 → ["chat_gpt"]。
# 只输出 JSON 数组，如 ["multi_format_agent"]。"""

#     llm_resp = await oxy_request.call(
#         callee="default_llm",
#         arguments={"messages": [{"role": "user", "content": prompt}]}
#     )
#     raw = getattr(llm_resp, "output", "[]")
...
#     """
#     # 使用绝对路径
#     TEST_DIR_ABS = r"D:\传输文件\数据挖掘实验\compete-main\OxyGent-main\test"
...
#                         "query": full_query,
#                         "callee": "master_agent",
#                         "shared_data": {"query": full_query, "file_name": file_name}
#                     }
...
async def intent_workflow(oxy_request: OxyRequest):
    ...
    # 新的意图识别逻辑 - 优先直接工具，最后才用搜索
    prompt = f"""你是意图识别助手，请根据用户输入决定下一步调用哪个智能体。
输入：{query}
文件：{file_name}

【优先级规则】：
1. 如果提到了文件/路径 → ["multi_format_agent"]
2. 如果包含完整URL或网址 → ["web_agent"] 
3. 如果是时间问题 → ["time_agent"]
4. 如果是库存问题 → ["inventory_agent"]
5. 如果是订单问题 → ["delivery_agent"]
6. 如果包含GitHub相关内容 → ["github_agent"]
7. 如果是需要实时搜索且没有其他明确工具的问题 → ["external_search_agent"]
8. 其他情况 → ["chat_gpt"]

只输出 JSON 数组，如 ["web_agent"]。"""
...
5. 不要包含"数据来源"等说明性文字。
6. 答案不包含双引号
7. 问题问什么，答案就只回答什么，回答核心结果，不要多余赘述
"""
